...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  const repo = config.localRepo ? "local checkout" : config.github.repo;
  return (
    <html lang="en">
      <body>
        <nav className="nav">
          ...
          <Link href="/dag">Causal graph</Link>
          <Link href="/browse?path=.claude/SKILLS.md">Skills</Link>
          <Link href="/upload">Upload</Link>
          <span className="spacer" />
          <span className="muted mono nav-meta">{repo ? repo : "no repo configured"} · {config.model}</span>
